...
const MemberPage = () => {
  ...
  const handleViewDocument = async (documentObj) => {
    try {
      ...
      // Release URL object after short time (not critical if tab remains open)
  ...
  return (
    <div className="space-y-4 w-full max-w-full mx-auto pb-safe-nav sm:pb-28">
      {/* Confetti Animation */}
      {showConfetti && (
        <Confetti
          key={confettiKey}
      ...
      <MemberTabBar activeTab={activeTab} onTabChange={handleTabChange} />
    </div>
  );
};
